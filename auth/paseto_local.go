...
func (p *LocalPaseto) makeToken(subject, audience string, typ LocalTokenType, ttl time.Duration, key psto.V4SymmetricKey, customClaims map[string]any) (string, error) {
	now := time.Now().UTC()
	exp := now.Add(ttl)
